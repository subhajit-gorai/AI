...
def get_emmission_prob(tag, word, emmission_given_tag, count_by_tag, total_vocabulary_words, laplace_lambda):
    ...
    if denominator == 0:
        print(f" /0 exception {count_by_tag[tag] }")
    return np.log(numerator/denominator)
...
def predict_pos_tag(line, total_lines, prob_sent_start_with_tag, tag_inverse_index, transition_prob, count_by_tag, total_number_of_tags, emmission_given_tag, total_vocabulary_words, laplace_lambda):
    dp = {}
    size = len(line)
    i = 0
        ...
        if word != "START" and word != "END":
            if i == 1: # first word
                for j in range(total_number_of_tags):
                    ...
                    emit = get_emmission_prob(tag_inverse_index[j], word, emmission_given_tag, count_by_tag, total_vocabulary_words, laplace_lambda)
                    if i not in dp:
                        dp[i] = {}
                ...
                for j in range(total_number_of_tags):
                    ...
                    for k in range(total_number_of_tags):
                        ...
                        emit = get_emmission_prob(current_tag, word, emmission_given_tag, count_by_tag,
                                                  total_vocabulary_words, laplace_lambda)
...
def viterbi_1(train, test):
    ...
    tag_inverse_index = {}
    total_vocabulary_words = len(possible_words)
    ...
    test_tagging = []
    for line in test:
        sol = predict_pos_tag(line, total_lines, prob_sent_start_with_tag,tag_inverse_index, transition_prob, count_by_tag, total_number_of_tags,
                        emmission_given_tag, total_vocabulary_words, laplace_lambda)
